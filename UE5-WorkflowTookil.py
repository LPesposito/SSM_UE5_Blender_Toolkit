...
def find_image_node(input_pin):
    """Recursively finds the image node connected to a socket"""
    if not input_pin or not input_pin.is_linked:
        return None
    
    # Acesso seguro ao primeiro link
    ...
    # Busca nos inputs do nó intermediário
...
class UE5_OT_PrepareGeometry(Operator):
    """Fix Transforms and adjust Pivot"""
    ...
    def execute(self, context):
        ...
        if mode == 'CENTER':
            ...
            for obj in objs:
                ...
                # Optimized vertex bounds calculation
                ...
                if bsdf:
                    for pin, suffix in map_slots.items():
                        ...
                        if node and node.image:
                            node.image.name = f"T_{mat.name}_{suffix}"
        return {'FINISHED'}

class UE5_OT_SaveTextures(Operator):
    """Export textures to disk"""
...
class UE5_OT_ExportFBX(Operator, ExportHelper):
    """FBX Export with light multiplier"""
...
class VIEW3D_PT_UE5_Toolkit(Panel):
    ...
    def draw(self, context):
        ...
        box.prop(p, "light_intensity")
        box.prop(p, "apply_transforms")
        box.prop(p, "export_individual")
        box.separator()
